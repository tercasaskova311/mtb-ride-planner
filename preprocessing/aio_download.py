...
class SumavaDownloader:
    ...
    def download_all(self):
        ...
        print("Downloading CHKO Šumava...")
        chko_gdf = self._download_nominatim("CHKO Šumava")
        if chko_gdf is not None:
            chko_gdf.to_file(self.output_dir / "sumava_chko.geojson", driver="GeoJSON")
            print("✅ CHKO Šumava downloaded")

        return np_gdf, chko_gdf

    def preview_map(self):
        geojsons = list(self.output_dir.glob("*.geojson"))
        if not geojsons:
            print("⚠️ No GeoJSON files to preview.")
            return
        fig, ax = plt.subplots(figsize=(12,10))
        for f in geojsons:
            gpd.read_file(f).plot(ax=ax, alpha=0.5, edgecolor='red', linewidth=2, label=f.stem)
        ax.set_title("Šumava NP & CHKO - Nominatim")
        ax.legend()
        plt.tight_layout()
        plt.show()
...
if __name__ == "__main__":
    ...
    np_gdf, chko_gdf = downloader.download_all()
